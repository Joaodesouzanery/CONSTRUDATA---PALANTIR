-- ============================================================================
-- APPLY_FINAL_v3.sql — Versão limpa. DROP POLICY como statements diretas
-- (não dentro de DO $$ block que pode falhar em alguns Supabase).
-- Cole INTEIRO no SQL Editor e Run.
-- ============================================================================

-- ═══════════════════════════════════════════════════════════════════════════
-- 1) FIX 0037 — Drop + Recreate policies
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Backfill profiles para users que não têm
DO $$
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM projects WHERE id=p_project_id AND organization_id=v_org AND deleted_at IS NULL) THEN RAISE EXCEPTION 'not found' USING ERRCODE='42501'; END IF;
