    ...
    <div className="p-6 space-y-5 overflow-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-gray-500">{items.length} lançamentos — Total: <strong className={tipo === 'entrada' ? 'text-emerald-400' : 'text-red-400'}>{fmtBRL(total)}</strong></p>
        </div>
        <button onClick={() => setShowAdd(true)}
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${tipo === 'entrada' ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30' : 'bg-red-500/20 text-red-400 hover:bg-red-500/30'}`}>
      ...
      {byCat.size > 0 && (
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setFilterCat('')} className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${!filterCat ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}>Todas</button>
          {cats.map((c) => {
            const val = byCat.get(c.key)
            if (!val) return null
            return <button key={c.key} onClick={() => setFilterCat(filterCat === c.key ? '' : c.key)}
              className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${filterCat === c.key ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}>
              {c.label} ({fmtBRL(val)})
            </button>
          })}
        </div>
      ...
      {/* Table */}
      {items.length === 0 ? (
        <div className="text-center py-12 text-gray-500 text-sm">Nenhum lançamento.</div>
      ) : (
            ...
            <tbody className="divide-y divide-[#1f2937]">
              {items.map((e) => (
                <tr key={e.id} className="hover:bg-white/[0.02] group">
                  <td className="px-3 py-2 text-gray-400 tabular-nums">{e.data}</td>
                  <td className="px-3 py-2 text-white">{e.descricao}</td>
                  <td className="px-3 py-2 text-gray-400 capitalize">{e.categoria.replace('_', ' ')}</td>
                  <td className="px-3 py-2 text-gray-500">{e.referencia || '—'}</td>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={descricao} onChange={(e) => setDescricao(e.target.value)} required placeholder="Ex: Medição Março/2026"
              className="w-full bg-[#111827] border border-[#1f2937] rounded px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[10px] text-gray-500 uppercase mb-1">Referência</label>
              <input value={referencia} onChange={(e) => setReferencia(e.target.value)} placeholder="NF-001, Med-03"
                className="w-full bg-[#111827] border border-[#1f2937] rounded px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500" />
            </div>
          </div>
        </div>
        <button type="submit" className="w-full py-2.5 rounded-lg text-xs font-semibold text-white" style={{ background: tipo === 'entrada' ? '#10b981' : '#ef4444' }}>
