    ...
    <div className="overflow-x-auto">
      <svg width={LABEL_W + W + 80} height={svgH} className="font-mono text-[10px]">
        ...
        {allDates.filter((_, i) => i % Math.max(1, Math.floor(totalDays / 8)) === 0).map((d) => {
          ...
          return (
            <g key={d}>
              <line x1={x} y1={0} x2={x} y2={svgH - 14} stroke="#525252" strokeWidth={0.5} />
        ...
        {rows.map((row, i) => {
          ...
          const rowBg = i % 2 === 0 ? '#48484808' : 'transparent'
    ...
    <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4">
      ...
      <div className="overflow-x-auto">
        <svg width={W} height={H} className="font-mono text-[10px]">
          ...
          {yTicks.map((t, i) => (
            <g key={i}>
              <line x1={PAD_L} y1={t.y} x2={W} y2={t.y} stroke="#525252" strokeWidth={0.5} strokeDasharray="4,3" />
...
function DelaySummaryInfographic({
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4 flex items-center justify-center h-24">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4 text-center flex flex-col gap-1">
...
export function SimulacaoAtrasoPanel() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {base && (
        <div className="flex items-center gap-2 text-xs text-[#6b6b6b] bg-[#2c2c2c] rounded-lg px-3 py-2 border border-[#525252]">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4 flex flex-col gap-3 lg:w-80 shrink-0">
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 bg-[#2c2c2c] border border-[#525252] rounded-lg px-2 py-1.5 text-xs text-[#f5f5f5] focus:outline-none focus:border-[#f97316]/60"
            >
            ...
            <input
              ...
              className="w-14 bg-[#2c2c2c] border border-[#525252] rounded-lg px-2 py-1.5 text-xs text-[#f5f5f5] text-center focus:outline-none focus:border-[#f97316]/60"
            />
            <button
              onClick={addDelay}
              disabled={!selCode}
          ...
          {delays.length === 0 ? (
            ...
            <div className="flex flex-col gap-1.5">
              {delays.map((d) => (
                <div key={d.trechoCode} className="bg-[#484848] rounded-lg px-3 py-2">
                  {editingCode === d.trechoCode ? (
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="w-14 bg-[#2c2c2c] border border-[#f97316]/60 rounded px-1.5 py-0.5 text-xs text-[#f5f5f5] text-center focus:outline-none"
                        autoFocus
                      />
...
function KpiCard({ icon, label, value, accent }: {
  ...
  return (
    <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl px-3 py-3 flex items-center gap-2">
      <div className="flex items-center justify-center w-7 h-7 rounded-lg shrink-0 bg-[#484848]">
        {icon}
      </div>
