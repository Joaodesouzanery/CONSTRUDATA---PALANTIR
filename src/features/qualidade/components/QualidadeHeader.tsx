/**
 * QualidadeHeader — top navigation and action bar for the Qualidade module.
 * Mesma identidade visual do RdoHeader (tema dark com accent laranja).
 * Reutiliza LogoConfigModal do módulo RDO (logos compartilhadas via companySettingsStore).
 */
import { useState } from 'react'
import { ShieldCheck, Plus, Download, Settings } from 'lucide-react'
...
const TABS: { key: FvsTab; label: string }[] = [
  ...
  { key: 'historico', label: 'Histórico de FVS' },
]
...
export function QualidadeHeader() {
  const { activeTab, setActiveTab, fvss } = useQualidadeStore()
  ...
  function handleExportCsv() {
    ...
    a.href = url
    a.download = `fvs-${new Date().toISOString().slice(0, 10)}.csv`
      ...
      <div className="px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#a3a3a3] text-xs">Ficha de Verificação de Serviço (FVS)</p>
          </div>
        </div>
