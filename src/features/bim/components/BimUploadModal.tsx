...
import { X, Upload, FileCheck, FileText } from 'lucide-react'
import { useBimStore } from '@/store/bimStore'
import { cn } from '@/lib/utils'

type ModalTab = 'shapefile' | 'levantamento'
...
export function BimUploadModal({ onClose }: { onClose: () => void }) {
  ...
  const [tab, setTab]         = useState<ModalTab>('shapefile')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-[520px] p-5 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-gray-100 font-semibold text-sm">Importar Dados Espaciais</h2>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-4 bg-gray-800 rounded-lg p-0.5">
          {([
        ...
        {tab === 'shapefile' && (
          <>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-gray-200 text-sm">Cancelar</button>
              <button
        ...
        {tab === 'levantamento' && (
          <>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-3 mb-3">
              <p className="text-gray-400 text-xs font-semibold mb-1">Formato esperado (CSV/TXT):</p>
              <p className="text-gray-500 text-[11px] font-mono">número, nome, northing, easting, profundidade</p>
            ...
            <button
              ...
              className="flex items-center gap-2 w-full justify-center px-3 py-3 border-2 border-dashed border-gray-700 hover:border-indigo-500 rounded-lg text-gray-400 hover:text-indigo-300 text-sm transition-colors"
            >
              <FileText size={16} />
            ...
            {surveyPreview.length > 0 && (
              <div className="mt-3 bg-gray-800 rounded-lg p-2">
                <p className="text-gray-500 text-[10px] font-semibold uppercase tracking-wider mb-1">Prévia (5 primeiras linhas)</p>
                <div className="space-y-0.5">
                  {surveyPreview.map((line, i) => (
                    <p key={i} className="text-gray-400 text-[11px] font-mono truncate">{line}</p>
                  ))}
                </div>
              </div>
            )}
