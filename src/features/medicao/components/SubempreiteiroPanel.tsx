/**
 * SubempreiteiroPanel — Subempreiteiro measurement tab.
 * Same as Sabesp but with subempreiteiro selection/filter at the top.
 */
...
export function SubempreiteiroPanel() {
  const {
    sheets,
    addSheet,
    updateSheet,
  ...
  const activeSheet =
    subSheets.find((s) => s.id === activeSheetId) ?? subSheets[0] ?? null

  // Collect all unique subempreiteiro names across sheets
  const allSubempreiteiros = useMemo(() => {
    const set = new Set<string>()
    subSheets.forEach((s) => {
      if (s.subempreiteiro) set.add(s.subempreiteiro)
    })
  ...
  // Further filter sheets by subempreiteiro if filter is active
  const displaySheets = subFilter
    ? subSheets.filter((s) => s.subempreiteiro === subFilter)
    : subSheets

  const handleCreateSheet = useCallback(() => {
    const id = addSheet({
      tipo: 'subempreiteiro',
      titulo: `Subempreiteiro ${subSheets.length + 1}`,
      referencia: new Date().toISOString().slice(0, 7),
      subempreiteiro: '',
      items: [],
    })
    setActiveSheetId(id)
  }, [addSheet, subSheets.length])

  const handleSubempreiteiroChange = useCallback(
    (value: string) => {
      if (!activeSheet) return
      updateSheet(activeSheet.id, { subempreiteiro: value })
    },
  ...
  return (
    <div className="p-6 space-y-4">
      {/* Subempreiteiro filter */}
      <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <Users size={16} className="text-[#f97316]" />
            <span className="text-[#f5f5f5] text-sm font-medium">Subempreiteiro</span>
          </div>
          <div className="flex items-center gap-2 flex-1">
            <select
              value={subFilter}
              onChange={(e) => setSubFilter(e.target.value)}
              className="bg-[#2c2c2c] border border-[#525252] rounded-lg text-xs text-[#f5f5f5] px-3 py-2 min-w-[200px]"
            >
              <option value="">Todos os subempreiteiros</option>
              {allSubempreiteiros.map((s) => (
            ...
            {activeSheet && (
              <input
                type="text"
                placeholder="Nome do subempreiteiro desta planilha..."
                value={activeSheet.subempreiteiro || ''}
                onChange={(e) => handleSubempreiteiroChange(e.target.value)}
                className="flex-1 bg-[#2c2c2c] border border-[#525252] rounded-lg text-xs text-[#f5f5f5] px-3 py-2 placeholder:text-[#6b6b6b]"
              />
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto">
          {displaySheets.map((s) => (
            ...
            >
              ...
              {s.titulo}
              {s.subempreiteiro && (
                <span className="text-[10px] opacity-70">({s.subempreiteiro})</span>
              )}
            </button>
          ))}
        </div>
