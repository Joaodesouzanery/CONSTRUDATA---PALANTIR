/**
 * FornecedorPanel — Fornecedor measurement tab.
 * Same as Sabesp but with fornecedor selection/filter at the top.
 */
...
export function FornecedorPanel() {
  const {
    sheets,
    addSheet,
    updateSheet,
  ...
  const fornecedorSheets = sheets.filter((s) => s.tipo === 'fornecedor')

  const [activeSheetId, setActiveSheetId] = useState<string | null>(
    fornecedorSheets[0]?.id ?? null,
  )
  ...
  const activeSheet =
    fornecedorSheets.find((s) => s.id === activeSheetId) ?? fornecedorSheets[0] ?? null

  const allFornecedores = useMemo(() => {
    const set = new Set<string>()
    fornecedorSheets.forEach((s) => {
      if (s.fornecedor) set.add(s.fornecedor)
    })
  ...
  const displaySheets = fornecedorFilter
    ? fornecedorSheets.filter((s) => s.fornecedor === fornecedorFilter)
    : fornecedorSheets

  const handleCreateSheet = useCallback(() => {
    const id = addSheet({
      tipo: 'fornecedor',
      titulo: `Fornecedor ${fornecedorSheets.length + 1}`,
      referencia: new Date().toISOString().slice(0, 7),
      fornecedor: '',
      items: [],
    })
    setActiveSheetId(id)
  }, [addSheet, fornecedorSheets.length])

  const handleFornecedorChange = useCallback(
    (value: string) => {
      if (!activeSheet) return
      updateSheet(activeSheet.id, { fornecedor: value })
    },
      ...
      <div className="bg-[#3d3d3d] border border-[#525252] rounded-xl p-4">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#f5f5f5] text-sm font-medium">Fornecedor</span>
          </div>
          <div className="flex items-center gap-2 flex-1">
            ...
            {activeSheet && (
              <input
                type="text"
                placeholder="Nome do fornecedor desta planilha..."
                value={activeSheet.fornecedor || ''}
        ...
        <div className="flex items-center gap-2 overflow-x-auto">
          {displaySheets.map((s) => (
            ...
            >
              ...
              {s.titulo}
              {s.fornecedor && (
                <span className="text-[10px] opacity-70">({s.fornecedor})</span>
              )}
            </button>
          ))}
        </div>
