...
function EditableCell({ value, onChange, type = 'text', min, max, step, className = '' }: {
  ...
  if (editing) {
    return (
      <input
        ...
        className={`w-full bg-[#2a2a2a] border border-orange-500 rounded px-2 py-1 text-sm text-white focus:outline-none ${className}`}
      />
    )
  }
  ...
  return (
    <span
      ...
      className={`cursor-text hover:bg-[#262626] rounded px-1 py-0.5 transition-colors text-sm text-[#f5f5f5] ${className}`}
    >
      {value}
    </span>
  )
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm bg-[#262626] hover:bg-[#2a2a2a] text-[#f5f5f5] transition-colors"
          >
            <Download size={14} />
          ...
          <tbody className="divide-y divide-[#1e1e1e]">
            ...
            {trechos.map((t, idx) => (
              <tr
                ...
                className="bg-[#0f0f0f] hover:bg-[#1e1e1e] transition-colors"
              >
                {/* Drag handle */}
                ...
                <td className="px-3 py-2">
                  ...
                  >
                    {Object.entries(SOIL_LABELS).map(([k, v]) => (
                      <option key={k} value={k} className="bg-[#1e1e1e]">{v}</option>
                    ))}
                  </select>
                </td>
