...
const NOTE_STATUS_META: Record<NoteStatus, { label: string; color: string }> = {
  pendente:     { label: 'Pendente',     color: 'bg-[#262626]/60 text-[#a3a3a3]' },
...
const TYPE_META: Record<NoteType, { label: string; color: string; Icon: React.ElementType }> = {
  ...
  other:        { label: 'Outro',       color: 'bg-[#262626]/60 text-[#f5f5f5] border-[#363636]/50',   Icon: Info },
}
...
function NoteDialog({ initial, trechos, teams, onClose, onSave }: NoteDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1e1e1e] rounded-2xl border border-[#363636] w-full max-w-lg shadow-2xl">
        ...
        <div className="px-6 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <Field label="Tipo" error={errors.type}>
              <select value={form.type} onChange={(e) => update('type', e.target.value as NoteType)}
                ...
                {Object.entries(TYPE_META).map(([k, v]) => (
                  <option key={k} value={k} className="bg-[#1e1e1e]">{v.label}</option>
                ))}
              </select>
            </Field>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Trecho" error={errors.trechoId}>
              <select value={form.trechoId} onChange={(e) => update('trechoId', e.target.value)}
                className="w-full bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500">
                {trechos.map((t) => (
                  <option key={t.id} value={t.id} className="bg-[#1e1e1e]">{t.code} — {t.description.slice(0, 30)}</option>
                ))}
              </select>
            </Field>
          ...
          <Field label="Título" error={errors.title}>
            <input type="text" value={form.title} onChange={(e) => update('title', e.target.value)}
              ...
              className="w-full bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white placeholder-[#6b6b6b] focus:outline-none focus:border-orange-500" />
          </Field>
          ...
          <Field label="Conteúdo" error={errors.body}>
            <textarea value={form.body} onChange={(e) => update('body', e.target.value)}
              ...
              className="w-full bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white placeholder-[#6b6b6b] focus:outline-none focus:border-orange-500 resize-none" />
          </Field>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-[#2a2a2a]">
          <button onClick={onClose}
            className="flex-1 px-4 py-2 rounded-lg text-sm bg-[#262626] hover:bg-[#2a2a2a] text-[#f5f5f5] transition-colors">
            Cancelar
          </button>
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-[200px] bg-[#262626] border border-[#363636] rounded-lg px-3 py-2">
        ...
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)}
          ...
          {Object.entries(TYPE_META).map(([k, v]) => <option key={k} value={k} className="bg-[#1e1e1e]">{v.label}</option>)}
        </select>
        <select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)}
          className="bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500">
          <option value="">Todas as prioridades</option>
          {Object.entries(PRIORITY_META).map(([k, v]) => <option key={k} value={k} className="bg-[#1e1e1e]">{v.label}</option>)}
        </select>
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
          className="bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500">
          <option value="">Todos os status</option>
          {Object.entries(NOTE_STATUS_META).map(([k, v]) => <option key={k} value={k} className="bg-[#1e1e1e]">{v.label}</option>)}
        </select>
        <select value={filterTrecho} onChange={(e) => setFilterTrecho(e.target.value)}
          className="bg-[#262626] border border-[#363636] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500">
          <option value="">Todos os trechos</option>
          {trechos.map((t) => <option key={t.id} value={t.id} className="bg-[#1e1e1e]">{t.code}</option>)}
        </select>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((note) => {
          ...
          return (
            <div key={note.id} className="bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between text-xs text-[#6b6b6b] mt-auto pt-2 border-t border-[#2a2a2a]">
