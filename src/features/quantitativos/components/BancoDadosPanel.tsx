...
function AddEntryRow({ onAdd }: { onAdd: (e: Omit<CustomBaseEntry, 'id'>) => void }) {
  ...
  return (
    <>
      <tr className="border-b border-[#363636] bg-gray-750/50">
        ...
        {BASE_OPTIONS.map((opt) => (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${costBase === opt.value ? 'border-violet-500 bg-violet-500' : 'border-[#363636]'}`}>
