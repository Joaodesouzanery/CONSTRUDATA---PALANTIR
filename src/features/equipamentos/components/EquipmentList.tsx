...
export function EquipmentList() {
  ...
  const contractorOptions = Array.from(new Set(equipamentos.map((eq) => eq.contractorName).filter(Boolean))).sort()
  ...
  const filtered = equipamentos.filter((eq) => {
    ...
    const matchSearch =
      ...
      (eq.contractorName ?? '').toLowerCase().includes(term) ||
      (eq.siteName ?? '').toLowerCase().includes(term)
    const matchStatus = statusFilter === 'all' || eq.status === statusFilter
    const matchContractor = contractorFilter === 'all' || eq.contractorName === contractorFilter
    ...
    <>
      ...
      <div className="flex flex-col gap-2 p-3 border-b border-[#525252] shrink-0">
        <div className="relative">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#a3a3a3] pointer-events-none" />
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar por nome, código ou obra..."
            className="w-full bg-[#2c2c2c] border border-[#525252] rounded-lg pl-8 pr-3 py-2 text-xs text-[#f5f5f5] placeholder:text-[#8a8a8a] outline-none focus:border-[#f97316] transition-colors"
          />
        </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  active
                    ...
                    : 'border-[#525252] text-[#a3a3a3] hover:border-[#1f3c5e] hover:text-[#f5f5f5]'
                )}
              >
                {dotColor && !active && (
                  <span
                ...
                {f.label}
                <span className={cn('font-mono', active ? 'opacity-80' : 'text-[#8a8a8a]')}>
                  {count}
                </span>
              </button>
            )
        ...
        <select
          ...
          className="w-full rounded-lg border border-[#525252] bg-[#2c2c2c] px-3 py-2 text-xs text-[#f5f5f5] outline-none focus:border-[#f97316]"
        >
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
        {filtered.length === 0 ? (
          <div className="flex-1 flex items-center justify-center py-16 text-xs text-[#8a8a8a]">
      ...
      {/* Footer count */}
      <div className="px-4 py-2 border-t border-[#525252] shrink-0 text-[10px] text-[#8a8a8a]">
