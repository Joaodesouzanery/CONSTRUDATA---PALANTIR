...
export function AlertsPanel() {
  ...
  if (allAlerts.length === 0) {
    return (
      <div className="flex items-center gap-2 px-6 py-3 border-t border-[#2a2a2a] bg-[#1a1a1a] shrink-0 text-xs text-[#3f3f3f]">
  ...
  return (
    <div
      className="border-t border-[#2a2a2a] bg-[#1a1a1a] shrink-0"
      style={{ overflow: 'hidden', maxHeight: open ? 220 : 44, transition: 'max-height 0.2s ease' }}
    >
      {/* Toggle header */}
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-6 h-11 hover:bg-[#1f1f1f] transition-colors shrink-0"
      >
