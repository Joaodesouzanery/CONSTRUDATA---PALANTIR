/**
 * ObraFoundryScene — cena isométrica line-art no estilo "value chain" da
 * Palantir Foundry, redesenhada para o universo de obras da ConstruData:
 * estrutura em execução, grua, estação de tratamento, terraplenagem,
 * suprimentos e equipe em ilhas conectadas por fluxos de dados sobre uma
 * grade isométrica tênue.
 *
 * variant="status"  → chips de status de campo (hero, ref. imagem 1)
 * variant="modules" → rótulos de módulo estilo Foundry (seção ontologia)
 */
...
function Box({
  x, y, z, w, d, h,
  top = TOP, right = RIGHT, left = LEFT, sw = 1.3,
...
/** Ilha circular (plataforma) em coordenadas de tela. */
function Disc({ cx, cy, rx, t = 9, top = PAD }: { cx: number; cy: number; rx: number; t?: number; top?: string }) {
  const ry = rx * 0.5
  return (
    <g stroke={STROKE} strokeWidth={1.3}>
      <path d={`M ${cx - rx} ${cy} L ${cx - rx} ${cy + t} A ${rx} ${ry} 0 0 0 ${cx + rx} ${cy + t} L ${cx + rx} ${cy}`} fill={LEFT} />
...
function Tank({ x, y, r = 1.4, h = 1.2, open = false }: { x: number; y: number; r?: number; h?: number; open?: boolean }) {
  ...
  return (
    <g stroke={STROKE} strokeWidth={1.3}>
      ...
      {open && <ellipse cx={top[0]} cy={top[1]} rx={rx * 0.72} ry={ry * 0.72} fill={LEFT} strokeWidth={1} />}
    </g>
  )
}
...
function Person({ x, y, s = 1 }: { x: number; y: number; s?: number }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`} stroke={STROKE} strokeWidth={1.5} strokeLinecap="round" fill="none">
...
function PeoplePad({ cx, cy, rx = 34, n = 4 }: { cx: number; cy: number; rx?: number; n?: number }) {
  ...
  return (
    <g>
      <ellipse cx={cx} cy={cy} rx={rx} ry={rx * 0.46} fill={PAD} stroke={STROKE} strokeWidth={1.2} />
...
function StructureBuilding() {
  ...
  return (
    <g>
      <Box x={-0.9} y={-0.9} z={-0.5} w={8.8} d={8.8} h={0.5} top={PAD} />
      <Box x={0} y={0} z={0} w={7} d={7} h={0.45} />
      {cols.map(([px, py], i) => (
        <Box key={`c1-${i}`} x={px} y={py} z={0.45} w={0.42} d={0.42} h={2.1} right="#e9e2d2" left="#dcd3bd" sw={1.1} />
      ))}
      <Box x={0} y={0} z={2.55} w={7} d={7} h={0.4} />
      {cols.slice(0, 12).map(([px, py], i) => (
        <Box key={`c2-${i}`} x={px} y={py} z={2.95} w={0.42} d={0.42} h={2.1} right="#e9e2d2" left="#dcd3bd" sw={1.1} />
      ))}
...
function TowerCrane() {
  ...
  return (
    <g stroke={STROKE} strokeWidth={1.4} fill="none" strokeLinejoin="round" strokeLinecap="round">
      <Disc cx={0} cy={4} rx={26} t={7} />
      ...
      {/* tirantes */}
      <line x1={0} y1={JY - 30} x2={-90} y2={JY - 4} strokeWidth={1} />
      <line x1={0} y1={JY - 30} x2={52} y2={JY - 8} strokeWidth={1} />
      <line x1={0} y1={JY - 30} x2={0} y2={JY - 13} />
      {/* carrinho + gancho + carga descendo sobre a laje */}
      <line x1={-80} y1={JY - 2} x2={-80} y2={JY + 32} strokeDasharray="2 3" strokeWidth={1.1} />
      <circle cx={-80} cy={JY + 35} r={2.6} fill={TOP} />
      <g transform={`translate(-80 ${JY + 38})`}>
        <Box x={-0.45} y={-0.45} z={-0.4} w={0.9} d={0.9} h={0.55} sw={1} />
      </g>
    </g>
  )
}
...
function TreatmentPlant() {
  ...
  return (
    <g>
      ...
      <Box x={0.4} y={0.3} z={2.9} w={0.45} d={0.45} h={0.9} sw={1} />
...
/** Escavadeira sobre ilha circular com leira de solo (terraplenagem). */
function Excavator() {
  ...
  return (
    <g>
      <Disc cx={0} cy={34} rx={86} t={10} />
      <g transform="translate(-66 -12)">
        {/* esteiras */}
        <Box x={0} y={0} z={0} w={2.8} d={1.4} h={0.55} top="#e2dac6" right="#d4cab2" left="#c5b99e" />
        ...
        <polygon
          ...
          fill="#ffffff" stroke={STROKE} strokeWidth={0.9} opacity={0.85}
        />
        ...
        <g fill="none" strokeLinecap="round" strokeLinejoin="round">
          ...
          <polyline points={`${pivot[0]},${pivot[1]} ${elbow[0]},${elbow[1]} ${wrist[0]},${wrist[1]}`} stroke={STROKE} strokeWidth={1.4} />
          <circle cx={pivot[0]} cy={pivot[1]} r={2.4} fill={TOP} stroke={STROKE} strokeWidth={1.2} />
          <circle cx={elbow[0]} cy={elbow[1]} r={2.2} fill={TOP} stroke={STROKE} strokeWidth={1.2} />
        </g>
        <polygon
          points={pts([[wrist[0] - 4, wrist[1]], [tip[0] - 2, tip[1]], [tip[0] + 12, tip[1] - 2], [wrist[0] + 8, wrist[1] - 5]])}
          fill={LEFT} stroke={STROKE} strokeWidth={1.3} strokeLinejoin="round"
        />
      </g>
      {/* leira de solo escavado */}
      <g transform="translate(34 24)">
        <path d="M -26 8 Q -16 -14 4 -12 Q 26 -9 22 6 Q 12 15 -10 13 Q -23 13 -26 8 Z" fill={RIGHT} stroke={STROKE} strokeWidth={1.2} />
        <path d="M -12 -6 q 6 -4 12 -1 M -2 6 q 8 -3 14 -6" fill="none" stroke={STROKE} strokeWidth={0.9} opacity={0.5} />
      </g>
...
/** Caminhão basculante + estoque de tubos e pallets (suprimentos). */
function SupplyYard() {
  const wheelAt = (x: number, y: number) => iso(x, y, 0.28)
  return (
    <g>
      {/* via */}
      <polygon points={pts([iso(-1.5, 2.4, 0), iso(7.5, 2.4, 0), iso(7.5, 4.4, 0), iso(-1.5, 4.4, 0)])} fill="#efeadd" stroke={STROKE} strokeWidth={1.2} strokeLinejoin="round" />
      <polyline
        points={pts([iso(-1.1, 3.4, 0), iso(7.1, 3.4, 0)])}
        fill="none" stroke={STROKE} strokeWidth={1} strokeDasharray="7 7" opacity={0.5}
      />
      {/* caminhão */}
      <g transform="translate(8 -6)">
        <Box x={0.1} y={2.55} z={0.28} w={3.3} d={1.25} h={0.28} top="#e9e2d2" right="#ddd4bf" left="#cfc5ab" sw={1.1} />
        <Box x={0.15} y={2.55} z={0.56} w={2.3} d={1.25} h={0.85} />
        <Box x={2.55} y={2.55} z={0.56} w={0.85} d={1.25} h={1.05} />
        {[0.7, 1.8, 2.95].map((wx) => {
          const p = wheelAt(wx, 3.8)
          return <circle key={wx} cx={p[0]} cy={p[1]} r={4.6} fill="#3a3f36" stroke={STROKE} strokeWidth={1} />
        })}
      </g>
      {/* tubos empilhados + pallets */}
      <g transform="translate(-44 16)">
        <Box x={0} y={0} z={0} w={3} d={0.4} h={0.4} sw={1} />
        <Box x={0} y={0.5} z={0} w={3} d={0.4} h={0.4} sw={1} />
        <Box x={0} y={0.25} z={0.4} w={3} d={0.4} h={0.4} sw={1} />
      </g>
...
/** Estoque de materiais (pallets) sobre ilha. */
function MaterialPallets() {
  return (
    <g>
      <Disc cx={0} cy={16} rx={56} t={8} />
      <g transform="translate(-26 -4)">
        <Box x={0} y={0} z={0} w={1.25} d={1.25} h={0.16} sw={1} top="#e9e2d2" />
        <Box x={0.06} y={0.06} z={0.16} w={1.12} d={1.12} h={0.95} sw={1} />
        <Box x={1.6} y={0.3} z={0} w={1.25} d={1.25} h={0.16} sw={1} top="#e9e2d2" />
        <Box x={1.66} y={0.36} z={0.16} w={1.12} d={1.12} h={0.6} sw={1} />
        <Box x={0.7} y={1.7} z={0} w={1.1} d={1.1} h={0.7} sw={1} />
      </g>
...
function SiteOffice() {
  ...
  return (
    <g>
      <Box x={-0.7} y={-0.7} z={-0.5} w={4.8} d={4.2} h={0.5} top={PAD} />
      <Box x={0} y={0} z={0} w={3.4} d={2.8} h={2} />
      <Box x={0.5} y={0.45} z={2} w={0.75} d={0.75} h={0.4} sw={1} />
      <g stroke={STROKE} strokeWidth={0.9} opacity={0.6}>
        {[0.4, 1.5].map((yy) => {
      ...
      {/* antena de dados */}
      <g transform={`translate(${ant[0]} ${ant[1]})`} stroke={STROKE} strokeWidth={1.4} fill="none" strokeLinecap="round">
...
function StatusChip({ x, y, dot, text }: { x: number; y: number; dot: string; text: string }) {
  ...
  return (
    <g transform={`translate(${x} ${y})`} style={{ pointerEvents: 'none' }}>
      <rect x={0} y={-14} width={w} height={28} rx={14} fill="#ffffff" stroke="rgba(35,39,31,0.16)" strokeWidth={1} />
...
function ModuleLabel({ x, y, title, tag }: { x: number; y: number; title: string; tag: string }) {
  const w = Math.max(title.length * 7.4, tag.length * 6.6 + 14) + 20
  return (
    <g transform={`translate(${x} ${y})`} style={{ pointerEvents: 'none' }}>
      <rect x={0} y={-17} width={w} height={38} fill="#ffffff" stroke="rgba(35,39,31,0.14)" strokeWidth={1} />
      <text x={10} y={-2} fontFamily="'Space Grotesk', sans-serif" fontSize={11.5} fontWeight={700} letterSpacing={0.8} fill={STROKE}>
        {title.toUpperCase()}
      </text>
      <text x={10} y={12} fontFamily="'JetBrains Mono', monospace" fontSize={9} fontWeight={700} letterSpacing={1} fill={ACCENT}>
        {tag.toUpperCase()} ↘
      </text>
    </g>
  )
...
const statusChips: Array<{ x: number; y: number; dot: string; text: string }> = [
  ...
  { x: -212, y: 232, dot: '#23271f', text: 'Equipe · 24 em campo' },
]

const moduleLabels: Array<{ x: number; y: number; title: string; tag: string }> = [
  { x: -492, y: -212, title: 'Levantamento', tag: 'BIM' },
  { x: -496, y: -86, title: 'Canteiro / Execução', tag: 'RDO' },
  { x: 56, y: -252, title: 'Gestão / Diretoria', tag: 'Gestão 360' },
  { x: 310, y: -196, title: 'Terraplenagem', tag: 'Equipamentos' },
  { x: -150, y: 18, title: 'Rede de Esgoto — ETE', tag: 'Qualidade' },
  { x: 338, y: -56, title: 'Rede de Água — ETA', tag: 'Mapa' },
  { x: 300, y: 184, title: 'Suprimentos', tag: 'Compras' },
  { x: -188, y: 232, title: 'Medição', tag: 'Avanço' },
]
...
export function ObraFoundryScene({
  ...
  return (
    ...
    >
      ...
      {/* grade isométrica tênue */}
      <g stroke={GRID} strokeWidth={1} transform="translate(0 14)">
        {Array.from({ length: 27 }, (_, i) => i - 13).map((g) => {
          const a = iso(g, -13, 0)
          const b = iso(g, 13, 0)
          const c = iso(-13, g, 0)
          const d = iso(13, g, 0)
      ...
      {/* fluxos de dados entre ilhas (tracejados) */}
      <g fill="none" stroke={STROKE} strokeWidth={1.4} strokeDasharray="2 6" strokeLinecap="round" opacity={0.55}>
      ...
      <circle className="fs-pulse" cx={-117} cy={58} r={4} fill={ACCENT} />
      <circle className="fs-pulse2" cx={140} cy={-76} r={4} fill={ACCENT} />
      <circle className="fs-pulse3" cx={222} cy={40} r={4} fill={ACCENT} />
      <circle className="fs-pulse2" cx={-56} cy={92} r={4} fill={ACCENT} />
      <circle className="fs-pulse" cx={152} cy={102} r={4} fill={ACCENT} />
