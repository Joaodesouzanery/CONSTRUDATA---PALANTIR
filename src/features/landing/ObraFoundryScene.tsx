...
function StatusChip({ x, y, dot, text }: { x: number; y: number; dot: string; text: string }) {
  ...
  return (
    <g transform={`translate(${x} ${y})`} style={{ pointerEvents: 'none' }}>
      ...
      <text x={28} y={4} fontFamily="'Inter Tight', sans-serif" fontSize={12} fontWeight={600} fill={STROKE}>
        {text}
      </text>
    </g>
  )
  ...
  return (
    <g transform={`translate(${x} ${y})`} style={{ pointerEvents: 'none' }}>
      <text
        ...
        fontFamily="'Inter Tight', sans-serif" fontSize={13.5} fontWeight={700} letterSpacing={1.1} fill={STROKE}
      ...
      <text
        ...
        fontFamily="'IBM Plex Mono', monospace" fontSize={10.5} fontWeight={700} letterSpacing={1.2} fill={ACCENT}
  ...
  return (
    ...
    >
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce){ .fs-pulse,.fs-pulse2,.fs-pulse3{ animation:none; opacity:.8 } .fs-dots{ display:none } }
      `}</style>
      ...
      {/* pulsos viajando nas conexões (dados alimentando a base) */}
      <g className="fs-dots">
        {[
          ['M -250 86 C -190 116, -140 60, -64 36', '-4.2s'],
          ['M 56 -34 C 120 -70, 168 -86, 212 -76', '-3.1s'],
          ['M 96 14 C 188 48, 286 38, 344 28', '-2.3s'],
          ['M 74 56 C 140 96, 186 116, 222 132', '-1.4s'],
          ['M -16 52 C -40 86, -70 106, -96 122', '-0.6s'],
        ].map(([d, begin]) => (
          <circle key={d} r={3.2} fill={ACCENT}>
            <animateMotion dur="4.5s" begin={begin} repeatCount="indefinite" path={d} />
          </circle>
        ))}
      </g>
