...
export function GridBottomPanel() {
  ...
  if (!bottomPanelOpen) {
    return (
      <div className="h-8 bg-[#0f0f0f] border-t border-[#2a2a2a] flex items-center px-3 shrink-0">
  ...
  return (
    <div className="h-52 bg-[#0f0f0f] border-t border-[#2a2a2a] flex flex-col shrink-0">
      {/* Tab strip */}
      <div className="flex items-center border-b border-[#2a2a2a] overflow-x-auto shrink-0 bg-[#0a1628]">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveGridTab(tab.id)}
            className={[
              'flex items-center gap-1.5 px-3 py-2 text-xs whitespace-nowrap border-r border-[#2a2a2a] transition-colors shrink-0',
              activeGridTab === tab.id ? 'bg-[#1e1e1e] text-[#f5f5f5]' : 'text-[#6b6b6b] hover:text-[#a3a3a3]',
            ].join(' ')}
          >
            <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: tab.color }} />
            {tab.label}
            <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-[#2a2a2a] text-[#a3a3a3] text-xs">{tab.count}</span>
          </button>
        ))}
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-[#2a2a2a]/50">
              {weatherStations.map((w) => (
                <tr key={w.id} className="hover:bg-[#1e1e1e] transition-colors">
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-[#2a2a2a]/50">
              {hardeningPoints.map((h) => (
                <tr key={h.id} className="hover:bg-[#1e1e1e] transition-colors">
