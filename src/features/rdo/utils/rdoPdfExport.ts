...
// Mudou de casa: o gerador do relatório de período agora é `rdosReportExport.ts`. O que ficava
// aqui lia `services`, `trechos` e os contadores de `manpower` — campos que o RDO Compizzo grava
// vazios de propósito —, então imprimia "Nenhum serviço" para RDOs cheios de dado. O novo tem um
// corpo por template, resolve as fotos (esta versão era síncrona e nunca as baixava) e inclui os
// RDOs Sabesp.
