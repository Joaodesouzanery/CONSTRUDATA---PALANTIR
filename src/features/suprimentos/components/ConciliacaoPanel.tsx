...
function PORow({ po, matchStatus, onRegisterReceipt, onEditPO }: PORowProps) {
  ...
  return (
    <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-xl overflow-hidden">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-4 px-4 py-3 hover:bg-[#252525] transition-colors text-left"
      >
        ...
        <div className="border-t border-[#2a2a2a] p-4 flex flex-col gap-4">
                ...
                {po.items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-t border-[#2a2a2a]">
                      ...
                      <td className={cn('px-3 py-2 text-right tabular-nums', rcItem ? (qtyOk ? 'text-[#4ade80]' : 'text-[#f87171]') : 'text-[#3a3a3a]')}>
                        {rcItem ? rcItem.receivedQty.toLocaleString('pt-BR') : '—'}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums text-[#f5f5f5]">
                        {item.unitPrice.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </td>
                      <td className={cn('px-3 py-2 text-right tabular-nums', nfItem ? (priceOk ? 'text-[#4ade80]' : 'text-[#f87171]') : 'text-[#3a3a3a]')}>
          ...
          <div className="flex gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              <span className={cn('w-2 h-2 rounded-full', receipt ? 'bg-[#4ade80]' : 'bg-[#3a3a3a]')} />
            ...
            <div className="flex items-center gap-1.5">
              <span className={cn('w-2 h-2 rounded-full', invoice ? 'bg-[#4ade80]' : 'bg-[#3a3a3a]')} />
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onEditPO}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#3a3a3a] text-[#a3a3a3] hover:text-[#f5f5f5] hover:border-[#555] text-xs font-medium transition-colors"
            >
              Editar OC
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col gap-3 flex-1 overflow-y-auto">
      ...
      <div className="flex items-center justify-between shrink-0">
        <span className="text-[#6b6b6b] text-xs">{purchaseOrders.length} ordens de compra</span>
        <button
