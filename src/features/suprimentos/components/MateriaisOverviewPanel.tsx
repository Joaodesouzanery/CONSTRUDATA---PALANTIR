...
export function MateriaisOverviewPanel() {
  ...
  const filteredFAs = catFilter === 'Todas'
    ? frameworkAgreements
  ...
  const poCompliance = purchaseOrders.map((po) => ({
    ...po,
    ...
    <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 shrink-0">
        <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 flex flex-col gap-1">
              ...
              {(['anual', 'trimestral', 'mensal'] as const).map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    period === p ? 'bg-[#2abfdc]/20 text-[#2abfdc]' : 'text-gray-600 hover:text-gray-300',
                  )}
                >
          ...
          <p className="text-xl font-bold text-gray-100 tabular-nums">{fmt(totals[period])}</p>
        </div>

        <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 flex flex-col gap-1">
          <p className="text-gray-500 text-xs">Pedidos em Aberto</p>
        ...
        <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 flex flex-col gap-1">
          <p className="text-gray-500 text-xs">Lead Time Médio</p>
      ...
      {/* ── Bar chart ── */}
      <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 shrink-0">
        <div className="flex items-center justify-between mb-3">
          <p className="text-gray-300 text-sm font-semibold">Gastos Mensais por Categoria</p>
      ...
      <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 shrink-0">
        <div className="flex items-center justify-between mb-3">
          <p className="text-gray-300 text-sm font-semibold">Comparativo de Fornecedores</p>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {vendorRows.map((row) => {
                ...
                return (
                  <tr key={row.faCode} className="border-b border-gray-800 hover:bg-gray-700/20 transition-colors">
                    <td className="py-2 text-gray-200 font-medium">{row.supplier}</td>
                    ...
                    <td className="py-2 text-right tabular-nums text-gray-300">{row.leadTime} dias</td>
              ...
              {vendorRows.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-6 text-center text-gray-600">
                    Nenhum acordo marco encontrado para esta categoria
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* ── PO Compliance ── */}
      <div className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 shrink-0">
        <p className="text-gray-300 text-sm font-semibold mb-3">Conformidade com Acordos Marco</p>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-gray-500 border-b border-gray-700">
            ...
            <tbody>
              {poCompliance.map((po) => {
                const total = po.items.reduce((s, i) => s + i.totalPrice, 0)
                return (
                  <tr key={po.id} className="border-b border-gray-800 hover:bg-gray-700/20 transition-colors">
                    <td className="py-2 font-mono text-gray-300">{po.code}</td>
                    <td className="py-2 text-gray-200">{po.supplier}</td>
                    <td className="py-2 text-right tabular-nums text-gray-200">{fmt(total)}</td>
                    <td className="py-2 text-right text-gray-400">{po.issuedDate}</td>
                    <td className="py-2 text-center">
                      {po.covered ? (
