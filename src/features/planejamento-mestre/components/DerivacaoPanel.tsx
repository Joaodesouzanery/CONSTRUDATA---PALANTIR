...
function DetailModal({ da, onClose }: DetailModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl border border-[#303030] bg-[#1a1a1a] flex flex-col shadow-2xl"
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    status === s
                      ...
                      : 'bg-transparent text-[#6b6b6b] border-[#303030] hover:text-[#a3a3a3]'
                  )}
                >
                  {STATUS_TEXT[s]}
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-[#141414] border border-[#303030] rounded-lg px-3 py-2 text-xs text-[#f5f5f5] focus:outline-none focus:border-[#f97316]/60 resize-none"
            />
          </div>
        </div>
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-[#303030]">
          <button onClick={onClose} className="px-3 py-1.5 rounded-lg border border-[#303030] text-xs text-[#6b6b6b] hover:text-[#a3a3a3]">Cancelar</button>
...
function Cell({ da, onClick, actName }: CellProps) {
  if (!da) {
    return (
      <td
        className="border border-[#303030]/30 min-w-[88px] h-8"
  ...
  return (
    <td
      ...
      className={cn('border border-[#303030]/30 cursor-pointer transition-colors min-w-[88px] h-8 px-2 py-1 align-middle', bg, 'hover:brightness-125')}
      onClick={onClick}
    >
...
function SectionHeaderRow({ label, color, colSpan }: SectionHeaderProps) {
  return (
    <tr>
      <td
        colSpan={colSpan}
        className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest border border-[#303030]/50"
...
function PpcRow({ weeks, das }: PpcRowProps) {
  ...
  return (
    <tr className="bg-[#0d1c36]">
      <td className="px-3 py-1.5 text-[9px] font-bold text-[#6b6b6b] uppercase tracking-widest sticky left-0 bg-[#0d1c36] z-10 border border-[#303030]/30 whitespace-nowrap">
        PPC Semana
      </td>
      {weeks.map((w) => {
        const v = ppc(w)
        return (
          <td key={w} className="px-2 py-1.5 text-center border border-[#303030]/30">
            {v === null ? (
  ...
  return (
    <div className="flex flex-col gap-4 overflow-hidden h-full">
      ...
      <div className="flex items-center gap-3 flex-wrap shrink-0">
        ...
        <div className="flex items-center gap-1 bg-[#202020] border border-[#303030] rounded-lg p-0.5 ml-auto">
      ...
      {!hasData ? (
        ...
        <div className="overflow-auto flex-1 rounded-xl border border-[#303030]">
          <table className="border-collapse text-xs min-w-full">
            {/* Header */}
            <thead className="sticky top-0 z-20">
              <tr className="bg-[#0a1628]">
                <th className="px-3 py-2.5 text-left text-[#6b6b6b] font-semibold border border-[#303030]/50 min-w-[190px] sticky left-0 bg-[#0a1628] z-30">
                  Atividade
                </th>
                {allWeeks.map((w) => (
                  <th key={w} className="px-2 py-1.5 text-center border border-[#303030]/50 min-w-[88px]">
            ...
            <tbody>
              ...
              {aguaRows.length > 0 && (
                <>
                  ...
                  {aguaRows.map((row) => (
                    <tr key={row.masterActivityId} className="hover:bg-[#202020]/40 transition-colors">
                      <td className="px-3 py-1.5 border border-[#303030]/30 sticky left-0 bg-[#0d1117] z-10">
