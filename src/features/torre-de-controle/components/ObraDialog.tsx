...
export function ObraDialog() {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl rounded-2xl border border-[#1c3658] bg-[#0e1f38] flex flex-col shadow-2xl"
            ...
            {!isNew ? (
              confirmDelete ? (
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => setConfirmDelete(false)} className="text-xs px-2 py-1 rounded bg-[#162e50] text-[#a3a3a3] hover:bg-[#1c3658]">Não</button>
                </div>
              ) : (
...
function Section({ title, children }: { title: React.ReactNode; children: React.ReactNode }) {
  return (
    <fieldset className="flex flex-col gap-3">
      <legend className="text-[10px] uppercase tracking-widest text-[#6b6b6b] font-semibold mb-0.5 w-full pb-1 border-b border-[#1c3658]">{title}</legend>
      {children}
    </fieldset>
  )
}
